...
(function () {
  ...
  function terms(s) { return [lc(s.category)].concat(s.tags.map(lc)); }
  function matches(termList, listLc) {
    for (var i = 0; i < termList.length; i++) {
      var t = termList[i];
      for (var j = 0; j < listLc.length; j++) {
        var x = listLc[j];
        if (t === x || t.indexOf(x) !== -1 || x.indexOf(t) !== -1) return true;
      }
    }
    return false;
  }
  function score(s) {
    var t = terms(s), ints = state.profile.interests.map(lc), sc = 0;
    t.forEach(function (term) {
      for (var j = 0; j < ints.length; j++) {
        var x = ints[j];
        if (term === x || term.indexOf(x) !== -1 || x.indexOf(term) !== -1) { sc += 2; break; }
      }
    });
  ...
  function isBlocked(s) {
    ...
    var bl = state.profile.blocked.map(lc);
    return bl.length > 0 && matches(terms(s), bl);
  }
  function isVisible(s) { return !isBlocked(s) && state.filters.has(s.category); }
  function ranked() {
  ...
  function jitter(stories) {
    ...
    stories.forEach(function (s) {
      var k = s.lat.toFixed(1) + "," + s.lng.toFixed(1);
    ...
    Object.keys(groups).forEach(function (k) {
      ...
      g.forEach(function (s, i) {
        ...
        if (g.length > 1) {
          var ang = (2 * Math.PI * i) / g.length, r = 0.9;
          c.lat += Math.sin(ang) * r; c.lng += Math.cos(ang) * r;
        }
        out.push(c);
      });
    });
  ...
  function renderMarkers() {
    ...
    vis.forEach(function (s) {
      ...
      var m = L.circleMarker([s.lat, s.lng], {
        radius: PRIO_RADIUS[s.priority] || 6, color: color, weight: 2,
        fillColor: color, fillOpacity: 0.35, className: cls
      });
      m.on("click", function () { selectStory(s.id); });
      m.bindTooltip(s.title, { direction: "top", className: "mk-tip", opacity: 0.9 });
  ...
  function highlightMarker(id) {
    Object.keys(markerById).forEach(function (k) {
      var el = markerById[k].getElement && markerById[k].getElement();
      if (el) el.classList.toggle("sel", k === id);
    });
  }

  /* ---------- feed / filters / blocked ---------- */
  function renderFeed() {
    var feed = document.getElementById("feed"); feed.innerHTML = "";
    var list = ranked().filter(isVisible);
    list.forEach(function (s, i) {
      var item = document.createElement("div");
      item.className = "feed-item" + (s.id === state.selected ? " active" : "") +
        (reactionOf(s) === "interested" ? " interested" : "");
      ...
      item.innerHTML =
        ...
        ' <span class="feed-prio' + prioCls + '">' + esc(s.priority) + "</span></div></div>";
  ...
  function renderBlocked() {
    ...
    var bc = document.getElementById("blockedCount");
    box.innerHTML = ""; bc.textContent = state.profile.blocked.length;
    state.profile.blocked.forEach(function (t) {
      var tag = document.createElement("span");
      tag.className = "blocked-tag"; tag.textContent = t;
      tag.title = "Click to un-suppress";
  ...
  function updateCounts() {
    ...
    document.getElementById("countPriority").textContent =
      vis.filter(function (s) { return s.priority === "critical" || s.priority === "high"; }).length;
  ...
  function selectStory(id) {
    ...
    if (map && markerById[id]) map.panTo([byId(id).lat, byId(id).lng], { animate: true });
  }
  ...
  function setTristate(rxn) {
    document.querySelectorAll("#tristate .tri").forEach(function (b) {
      b.classList.toggle("on", b.getAttribute("data-state") === rxn);
    });
  }

  /* ---------- reactions (the learning loop) ---------- */
  function setReaction(state_) {
    var s = byId(state.selected); if (!s) return;
    var tagsCat = [s.category].concat(s.tags);
    if (state_ === "interested") {
      state.reactions[s.id] = "interested";
      addUnique(state.profile.interests, tagsCat);
      ...
      addUnique(state.profile.blocked, tagsCat);
      toast("TOPIC SUPPRESSED: " + s.category + " (+" + s.tags.length + " tags)", "bad");
    } else {
      delete state.reactions[s.id];
      // un-suppress topics this story owns
      state.profile.blocked = state.profile.blocked.filter(function (b) { return tagsCat.map(lc).indexOf(lc(b)) === -1; });
      document.getElementById("dDetail").hidden = true;
      toast("ASSESSMENT CLEARED");
    }
    saveReactions(); saveProfile(); setTristate(reactionOf(s));
    if (state_ === "blocked") closeDossier();
    renderAll();
  }
    ...
    fetch("/api/news?blocked=" + encodeURIComponent(bl), { headers: { accept: "application/json" } })
      ...
      .then(function (data) {
        ...
        if (incoming.length) {
          ...
          toast((data && data.message) || "NO LIVE SOURCE CONFIGURED. SHOWING CACHED INTEL.", "warn");
        }
      })
      .catch(function () { toast("LIVE UPLINK OFFLINE. SHOWING CACHED INTEL.", "warn"); })
      .then(function () {
        state.lastRefresh = new Date(); stamp(); btn.classList.remove("busy"); setLive("LIVE", false); renderAll();
      });
  }
  function setLive(label, busy) {
    var el = document.getElementById("liveLabel"); el.textContent = label;
    document.querySelector(".stat.live").style.color = busy ? "#ffb000" : "#4ade80";
  }
  function stamp() {
    var el = document.getElementById("lastRefresh");
    el.textContent = state.lastRefresh ? hhmm(state.lastRefresh) + "Z" : "SEED";
  }
  ...
  function clockTick() {
    var d = new Date();
    setText("clock", pad(d.getUTCHours()) + ":" + pad(d.getUTCMinutes()) + ":" + pad(d.getUTCSeconds()));
  }
  var toastTimer = null;
  ...
  function wire() {
    ...
    document.querySelectorAll("#tristate .tri").forEach(function (b) {
      b.onclick = function () { setReaction(b.getAttribute("data-state")); };
    });
  ...
  function init() {
    initMap(); wire(); clockTick(); setInterval(clockTick, 1000); stamp(); renderAll(); boot();
  }
